...
const OscilloscopeGraph = ({ data, color, label }: { data: number[], color: string, label: string }) => {
    ...
    const len = validData.length > 1 ? validData.length - 1 : 1;
    return (
        <div className="relative h-24 bg-black/20 rounded-lg overflow-hidden flex flex-col justify-end group border border-white/5">
             ...
             <svg className="absolute inset-0 w-full h-full p-2 pt-6" preserveAspectRatio="none">
                 <path d={`M 0 100 L ${validData.map((v, i) => `${(i / len) * 100} ${100 - ((v - min) / range) * 80}`).join(' L ')} L 100 100 Z`} fill={color} fillOpacity="0.1" />
                 <path d={`M ${validData.map((v, i) => `${(i / len) * 100} ${100 - ((v - min) / range) * 80}`).join(' L ')}`} fill="none" stroke={color} strokeWidth="2" vectorEffect="non-scaling-stroke" />
             </svg>
             <div className="absolute bottom-2 right-3 text-xs font-mono font-bold text-white">{validData[validData.length-1]?.toFixed(3)}</div>
        </div>
    )
}
      ...
      return (
        <div className="fixed inset-0 bg-black z-[100] flex flex-col items-center justify-center font-mono">
            ...
            <div className="w-64 h-1 bg-gray-800 rounded-full overflow-hidden">
                <div className="h-full" style={{ background: '#00f0ff', animation: 'bootLoad 2s ease-out forwards', width: '0%' }}></div>
            </div>
            <div className="mt-4 text-xs tracking-[0.3em] opacity-80" style={{ color: '#00f0ff' }}>INITIALIZING CORE</div>
        </div>
      )
  }
