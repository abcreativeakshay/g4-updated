...
export const generateSteps = async (config: MLModelConfig): Promise<AlgorithmStep[]> => {
    try {
        ...
        console.warn("Step generation failed (using fallback):", e?.message || "Quota/Network error");
        return [
            { title: "Data Initialization", description: "Initialize random weights and biases for all network parameters.", visualCue: "Glowing nodes appear" },
            { title: "Forward Pass", description: "Input data flows through each layer, computing activations.", visualCue: "Light pulses between layers" },
            { title: "Loss Calculation", description: "Compare predictions with ground truth using loss function.", visualCue: "Red error indicators" },
            { title: "Backward Pass", description: "Compute gradients via backpropagation to update weights.", visualCue: "Blue gradients flow backward" },
            { title: "Weight Update", description: "Update weights using optimizer to minimize loss.", visualCue: "Nodes pulse and adjust" }
        ];
    }
}
